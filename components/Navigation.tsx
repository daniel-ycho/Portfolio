  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-black transition-all duration-300">
      <div className="max-w-[1920px] mx-auto px-4 md:px-12 h-16 flex items-center justify-between">
        <div 
          className="font-serif font-bold text-xl tracking-tight cursor-pointer hover:opacity-70 transition-opacity"
          ...
          <div className="flex flex-col p-6 space-y-6">
            {navItems.map((item) => (
              <a 
                ...
                href={`#${item.id}`} 
                onClick={(e) => handleScroll(e, item.id)}
                className="font-sans text-lg uppercase tracking-widest font-bold text-premium-black hover:text-wsj-blue flex items-center justify-between group"
              >
                {item.label}
